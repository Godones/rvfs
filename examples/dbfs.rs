use std::sync::Arc;
use dbfs2::{DBFS_TYPE, init_dbfs};
use jammdb::DB;
use jammdb::memfile::{FakeMap, FileOpenOptions};
use rvfs::{do_mount, FakeFSC, FileFlags, FileMode, init_vfs, MountFlags, register_filesystem, vfs_mkdir, vfs_open_file, vfs_read_file, vfs_write_file};
...
fn main() {
    ...
    let _ = vfs_open_file::<FakeFSC>("/f1", FileFlags::O_RDWR | FileFlags::O_CREAT, FileMode::FMODE_WRITE | FileMode::FMODE_READ).unwrap();
    ...
    let file = vfs_open_file::<FakeFSC>("/dbfs/f1", FileFlags::O_RDWR | FileFlags::O_CREAT, FileMode::FMODE_WRITE | FileMode::FMODE_READ).unwrap();
    println!("file: {:#?}", file);

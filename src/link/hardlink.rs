use alloc::sync::Arc;
use spin::Mutex;
use crate::{find_file_indir, LookUpFlags, path_walk, PathType, StrResult};
...
pub fn vfs_unlink<T:ProcessFs>(name:&str)->StrResult<()>{
    ...
    // 判断是否是目录
    let inode = sub_dentry.lock().d_inode.clone();
    if inode.lock().mode == InodeMode::S_DIR{
...
pub fn vfs_link<T:ProcessFs>(old: &str, new: &str) -> StrResult<()> {
    ...
    let mkdir = inode.lock().inode_ops.link;
    mkdir(old_lookup_data.dentry.clone(), inode.clone(),target_dentry.clone())?;
    // 设置目录名
